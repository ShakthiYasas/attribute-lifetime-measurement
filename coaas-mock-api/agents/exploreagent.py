...
class RandomAgent(ExplorationAgent):
    ...
    @staticmethod
    def _choose_action(obs_list_len):
        return random.randint(0, obs_list_len - 1)

    def choose_action(self, observation:dict):
        rand_ent_idx = RandomAgent._choose_action(len(observation))
        rand_ent = list(observation.items())[rand_ent_idx]

        rand_att_idx = RandomAgent._choose_action(len(rand_ent['attributes']))
...
class MRUAgent(ExplorationAgent):
    ...
    @staticmethod
    def _choose_action(observation:dict):
...
class MFUAgent(ExplorationAgent):
    ...
    def choose_action(self, observation:dict):
        # Returns (entityid, attribute)
        return MFUAgent._choose_action(observation)
