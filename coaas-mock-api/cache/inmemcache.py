...
class InMemoryCache(CacheAgent):
    ...
    def save(self, entityid, cacheitems) -> None:
        if(entityid in self.entityhash):
            for key, value in cacheitems.items():
                if(key not in self.entityhash[entityid].freq_table):
                    self.entityhash[entityid].freq_table[key] = (0,[])
                self.entityhash[entityid][key] = value
        else:
            self.entityhash[entityid] = LimitedSizeDict()
            for key, value in cacheitems.items():
                self.entityhash[entityid].freq_table[key] = (0,[])
                self.entityhash[entityid][key] = value
    ...
    def evict(self, entityid) -> None:
        self.entityhash.move_to_end(entityid,last=False)
        self.entityhash.popitem(last=False)
        del self.entityhash.freq_table[entityid]
    
    # Read the entire cache
    def get_values(self) -> dict:
        return self.entityhash

    # Check if the entity is cached
    def _is_cached(self,entityid,attribute):
        return entityid in self.entityhash and attribute in self.entityhash[entityid]        
    ...
    def get_value_by_key(self,entityid,attribute):
        ...
        if(self._is_cached(entityid,attribute)):
            ...
            att_stat = list(self.entityhash[entityid].freq_table[attribute])
            att_stat[0]=+1
            att_stat[1].append(datetime.now())
            self.entityhash[entityid].freq_table[attribute] = tuple(att_stat)
            
            # Updating frequency table for entities
            ent_stat = list(self.entityhash.freq_table[entityid])
            ent_stat[0]=+1
            ent_stat[1].append(datetime.now())
            self.entityhash.freq_table[entityid] = tuple(ent_stat)

            return self.entityhash[entityid][attribute]
        else:
            return None
            
    # Retrive frequency of access statistics for currently cached entities
    def get_statistics(self):
        return self.entityhash.freq_table

    # Retrive frequency of access statistics for a context attribute
    def get_statistics(self, entityid, attribute):
        return self.entityhash[entityid].freq_table[attribute]
